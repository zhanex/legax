...
| 想要手机访问，但不想暴露终端控制。 | 使用 relay 配对、Telegram 按钮和 adapter API，而不是抓取 UI。 |
...
| --- | --- | --- | --- | --- |
| 手机或浏览器远程控制 | 受限于原生界面 | 没有通用手机 relay | 没有通用手机 relay | Web UI、Telegram、webhook |
...
| 手机传输 | 自托管 relay Web UI、Telegram Bot API、outbound webhook 通知 |
| 原生审批 | Codex JSON-RPC、Claude permission-prompt MCP、Gemini CLI approval mode |
| 会话路由 | 在 relay 和 Telegram 菜单中选择 CLI、项目/聊天和会话 |
...
| Claude Code 示例 | [examples/claude-code/README.zh-CN.md](examples/claude-code/README.zh-CN.md) |
...
项目维护者不运营托管后端、共享 relay 或共享 Telegram bot。数据流向由你通过 transport 配置决定。
