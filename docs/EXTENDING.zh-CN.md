...
Legax 优先用结构化 CLI 协议，而不是抓 TUI 屏幕。优先级从高到低：
...
- poll 循环：被 daemon 启动时（`LEGAX_DAEMON_ROUTER=1`）只 drain `pollInboundTransports(...)`；单独运行 adapter 时再 fallback 到直接轮询 `/api/messages`。
