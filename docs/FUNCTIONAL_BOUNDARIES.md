...
This document describes Legax from a business workflow perspective. It defines what the product does, which component owns each responsibility, and how the web relay and Telegram interactions close the loop from a remote user action back to a local CLI agent.
...
Legax is not a multi-user SaaS, not a hosted agent runtime, not a terminal UI automation layer, and not a way to bypass native CLI security policy. Process lifecycle belongs to the daemon and adapters. Remote access belongs to relay, Telegram, and webhook transports. Agent capabilities belong to MCP tools only when the host agent explicitly uses them.

## Actors

- Operator: owns the workstation, configures secrets, starts the relay and daemon, and pairs browser devices.
- Remote user: uses the relay web page or Telegram bot to choose a conversation target and send actions.
...
- `config.yaml`: local operator configuration. It is the source for relay URL, relay secret, Telegram bot token, webhook URLs, and adapter settings.
...
5. Operator runs the relay and daemon.
...
Goal: keep actionable work visible across browser and Telegram without duplicating per-transport logic.
...
4. Daemon polls relay `/api/messages` and Telegram `getUpdates`.
...
5. User clicks one action.
6. Relay or Telegram sends a `permission_decision` message.
7. Adapter waits for that decision and returns it through the CLI's native approval channel.
...
4. User answers through the relay composer or Telegram command.
...
- A new operator can configure relay, Telegram, and at least one CLI from the examples.
