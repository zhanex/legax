...
本软件用于把桌面编码 Agent CLI 桥接到手机端通道（self-hosted relay、Telegram bot、webhook），**面向单个运维者自己的桌面会话**。常见用例：
...
- 禁止在 fork、截图、bug 报告中再分发 Telegram bot token、relay 密钥或其他凭据。一旦暴露请先轮换。
...
启用第三方 transport（Telegram Bot API、webhook 端点）时，相关使用同时受其自身条款约束（例如 [Telegram 服务条款](https://telegram.org/tos)）。Legax 不减损、不改变这些条款。
