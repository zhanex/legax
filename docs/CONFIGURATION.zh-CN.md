...
- 顶层 scalar
- 两层嵌套 object
- 带缩进子 key 的 `transports:` list
- 每个 key 下的 scalar list
- 引号外 inline comment
- boolean、null、number、单引号字符串、双引号字符串和未加引号字符串

不支持 anchor、tag、多行 scalar、flow style 和任意深度嵌套。新配置形状必须适配这个子集；否则先同步扩展共享 parser 和独立 relay 副本。
...
| `audit.textPreview` | number | 预览长度；`0` 表示不包含正文预览。 |
...
| `webhook` | `url` | 可选 `secret` 按共享 transport 代码发送给接收服务。 |
