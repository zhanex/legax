...
- 英文文档是 canonical contract。简体中文文档是给偏好中文的读者使用的本地化镜像。
- 不要只在中文镜像中新增规则、配置字段、行为或兼容承诺；英文 canonical 文件必须包含同一契约。
- 编码 Agent 的普通任务只加载一种语言。只有翻译、配对一致性或分歧审查时才加载双语配对。
