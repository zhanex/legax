...
7. Daemon 轮询 relay `/api/commands`，claim 符合条件且在 allowlist 内的命令，在本机执行安全内置动作，并用当前 claim token 上报 terminal result。
...
- Daemon 会注册为 relay host，心跳过期后变为 offline，并且只执行已知 relay command refs，结果通过 claim token 上报。
