...
- `data/relay-store.json`：relay 拥有的状态，使用 `legax.relay/1` schema。保存 sessions、generations、leases、hosts、devices、transports、inbox、commands、metadata events、artifacts、workflow definitions/runs、legacy event/message 队列、配对码和浏览器配对状态。
