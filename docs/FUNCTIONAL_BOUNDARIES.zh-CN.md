...
本文从业务流程角度描述 Legax：项目负责什么、不负责什么，各组件的职责边界在哪里，以及 relay 网页和 Telegram 如何把远端用户的一次操作闭环到本地 CLI Agent。

## 产品边界

Legax 是面向编码 Agent CLI 的本地优先远程交互层。用户离开工作站后，仍然可以看到关键 Agent 事件、选择 CLI/项目/session、回复 Agent，并对原生权限请求做同意或拒绝。

Legax 不是多用户 SaaS，不是托管 Agent 运行时，不是终端 UI 自动化层，也不是绕过 CLI 原生安全策略的通道。进程生命周期归 daemon 和 adapter；远端访问归 relay、Telegram、webhook 等通讯通道；MCP 只在宿主 Agent 显式调用时提供能力工具。

## 参与角色

- 操作者：拥有工作站，负责配置密钥、启动 relay 和 daemon、配对浏览器设备。
- 远端用户：通过 relay 网页或 Telegram bot 选择会话目标并发送动作。
...
- Telegram transport：格式化出站通知，把入站消息和 inline 按钮转换成与 relay 相同的消息模型。
- MCP server：暴露通知、轮询和权限工具。它不负责启动或停止 CLI 进程。

## 数据归属

- `config.yaml`：本机操作者配置。保存 relay URL、relay secret、Telegram bot token、webhook URL 和 adapter 设置。
- `data/runtime-state.json`：daemon 与 adapter 的协同状态。保存游标、已选 session、模式、inbox 队列和启动请求。
- `data/relay-store.json`：relay 状态。保存事件、消息、已配对浏览器设备、配对码和可选审计记录。
...
目标：让浏览器和 Telegram 共享同一套待处理事项模型，而不是各自重复拼接逻辑。
...
4. Daemon 轮询 relay `/api/messages` 和 Telegram `getUpdates`。
5. Daemon 把入站消息按目标写入 per-agent inbox。
6. 如果用户选择的 adapter 未运行且 `daemon.launchOnDemand` 已启用，daemon 会记录启动请求并启动该 adapter。

闭环标准：即使某个 CLI adapter 尚未启动，远端菜单动作也能触发它按需启动。
...
5. 用户点击其中一个动作。
6. Relay 或 Telegram 发送 `permission_decision` 消息。
7. Adapter 等待该决定，并通过 CLI 原生审批通道返回。
...
4. 用户通过 relay 输入框或 Telegram 命令回答。
...
1. 用户发送 `/start`。
2. Bot 返回已启用 CLI 按钮。
3. 用户点击某个 CLI。Daemon 在需要时启动 adapter，并返回 project/chat 按钮。
4. 用户点击某个 project/chat。Adapter 返回该范围内的 session 按钮；已归档 session 不显示；没有项目元数据的 session 显示在 **Chats** 下；Claude cwd-only session 也会出现在 **Chats** 下；Codex app-server session 在没有显式 project 元数据时也显示在 **Chats** 下；OpenCode session 在 server 返回 cwd/project metadata 时按项目展示。超过 10 条时，回复包含 **Previous** / **Next** 翻页按钮。
5. 用户点击某个 session。Adapter 标记该 session 为当前选择。
6. 用户发送普通文本。Daemon 路由到已选 CLI/session。
7. Adapter 把完成结果、已支持的权限请求和用户输入请求通过 Telegram 发回。
...
- Telegram transport 负责 Telegram API 格式化和 callback 解析，不负责 adapter 生命周期。
...
- 新操作者可以根据示例配置 relay、Telegram 和至少一个 CLI。
...
- Telegram 可以完成 `/start -> CLI -> project/chat -> session -> text -> response`。
