...
Legax 是面向编码 Agent CLI 的本地优先远程交互层。用户离开工作站后，仍然可以看到关键 Agent 事件、选择 CLI/项目/session、回复 Agent，并对原生权限请求做同意或拒绝。

Legax 不是多用户 SaaS，不是托管 Agent 运行时，不是终端 UI 自动化层，也不是绕过 CLI 原生安全策略的通道。进程生命周期归 daemon 和 adapter；远端访问归 relay、Telegram、飞书/Lark、webhook 等通讯通道；MCP 只在宿主 Agent 显式调用时提供能力工具。

## 参与角色

- 操作者：拥有工作站，负责配置密钥、启动 relay 和 daemon、配对浏览器设备。
- 远端用户：通过 relay 网页、Telegram bot 或飞书/Lark 应用 bot 选择会话目标并发送动作。
- Daemon：监管已启用的 CLI adapter，轮询入站通道，路由消息，并创建按需启动请求。
- Adapter：负责一个 CLI 进程和该 CLI 的 session 模型。它列出项目/session、启动或恢复 session、把文本送入 CLI、解析结构化输出，并在 CLI 暴露受支持回调时镜像原生权限请求。
- Relay：保存事件、入站消息、配对码、已配对设备和审计记录，支持一个或多个 `sessionId`。
...
- `data/relay-store.json`：relay 拥有的状态，使用 `legax.relay/1` schema。保存 sessions、generations、leases、handoffs、hosts、devices、transports、inbox、commands、metadata events、artifacts、workflow definitions/runs、legacy event/message 队列、配对码和浏览器配对状态。
- `data/mcp-state.json`：通用 MCP 工具状态。
- CLI 原生历史：Codex、Claude Code、Gemini、OpenCode 各自拥有 session 历史。Legax 只通过各 CLI 支持的接口读取或恢复。
...
目标：让浏览器、Telegram 和飞书/Lark 共享同一套待处理事项模型，而不是各自重复拼接逻辑。
...
2. Daemon 读取 `config.yaml`，校验 adapter contract，并输出已脱敏的 transport 摘要。
3. Daemon 启动 `autoStart: true` 的 adapter。
4. Relay 负责 Telegram `getUpdates` 轮询或 `/api/telegram/events` webhook，以及飞书/Lark 回调，然后把归一化动作写入 `/api/messages`。
5. Daemon 轮询 relay `/api/messages`，并把入站消息按目标写入 per-agent inbox。
6. Daemon 向 `/api/hosts` 发送心跳，携带 host id、groups、已启用 adapter 元数据和支持的 command refs。
7. Daemon 轮询 relay `/api/commands`，claim 符合条件且在 allowlist 内的命令，在本机执行安全内置动作，并用当前 claim token 上报 terminal result。LPS TDD action command 以文档和 evidence 为中心，永远不从 workflow definition 接收自由 shell 字符串；当 action 可能修改 workspace 文件时，必须持有 generation lease。
8. 如果用户选择的 adapter 未运行且 `daemon.launchOnDemand` 已启用，daemon 会记录启动请求并启动该 adapter。

闭环标准：即使某个 CLI adapter 尚未启动，远端菜单动作也能触发它按需启动，并且 relay 可以观察 daemon host 存活状态和 command queue 进度。

### 3A. 管理 Portable Session 执行

目标：让面向用户的任务身份可以跨 host 迁移，同时把 CLI 原生 session id 保持为 adapter 专属元数据。

1. Relay 通过 `POST /api/sessions` 创建或更新稳定 session。
2. Relay 通过 `POST /api/generations` 创建 generation。generation 记录 host、adapter、CLI 原生元数据、worktree 元数据、checkpoint 元数据和结果状态。
3. daemon host 通过 `POST /api/leases/claim` claim 执行归属。
4. lease holder 用当前 `hostId`、`fencingToken` 和 `leaseToken` 续租或释放 lease。
5. generation 更新必须来自当前 lease holder。过期 fencing token 返回 `409`。
6. handoff 记录 `requested -> checkpointed -> uploaded -> released -> claimed -> restored -> resumed`，也可以进入 `failed`。
7. fork 从既有 generation checkpoint 创建子 generation，且不修改父 generation。

闭环标准：relay 可以展示 portable session id、当前 generation、lease holder、handoff 历史和 fork lineage，而不会把 CLI 原生 id 当作主要身份。
...
闭环标准：只有存在 CLI 目标时输入框才可用；用户选择 session 后，具备 session 能力的 adapter 会先收到 `select_session`，再接收普通文本。
...
目标：在所选 adapter 暴露受支持回调时，镜像 CLI 原生审批，而不是绕过它。
...
1. 用户发送 `/start`。
2. Relay 接收 Telegram update，校验允许的 chat，去重，将其归一化写入 `/api/messages`，并在需要时 ack callback query。
3. Daemon 路由归一化消息，并把状态/菜单事件写回 relay。
4. Relay 将这些事件 fan-out 到 Telegram，因此 bot 返回已启用 CLI 按钮。
5. 用户点击某个 CLI。Daemon 在需要时启动 adapter，并通过同一条 relay fan-out 路径返回 project/chat 按钮。
...
1. project/chat 菜单中显示 **New project**。它首先是普通 Telegram callback，不是直接打开 Web App 的按钮。
...
- Relay 负责 HTTP 鉴权、事件/消息队列、浏览器 UI、协议化 pairing offer、设备、attention inbox 推导和审计。
- Relay 拥有的 Telegram transport 负责 Telegram API 格式化、轮询/webhook 入站、callback 解析、callback ack 和事件 fan-out，不负责 adapter 生命周期。
...
- Daemon 会注册为 relay host，心跳过期后变为 offline，并且只执行已知 relay command refs，结果通过 claim token 上报。`pr.create` 是可选能力，默认禁用，并且必须保持 draft-first。
