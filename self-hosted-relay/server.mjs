...
const DEFAULT_CONFIG_PATH = "/etc/legax-relay/config.yaml";
function resolveConfigPath() {
  ...
  return requested || DEFAULT_CONFIG_PATH; // existence is checked below
}

const CONFIG_PATH = resolveConfigPath();
const RAW_CONFIG = readYaml(CONFIG_PATH) ?? {};
...
const STORE_PATH = RELAY_CFG.storePath
  ? path.resolve(RELAY_CFG.storePath)
...
const MAX_EVENTS_PER_SESSION = Number(RELAY_CFG.maxEventsPerSession ?? 500);
const MAX_MESSAGES_PER_SESSION = Number(RELAY_CFG.maxMessagesPerSession ?? 500);
...
const AUDIT_LOG_PATH = AUDIT_CFG.path
  ? path.resolve(AUDIT_CFG.path)
  : "/var/lib/legax-relay/relay-audit.jsonl";
...
// When allowInsecureDev: true with no secret, keep the relay on loopback.
// Public unauthenticated relays would become remote-control endpoints for the
// attached agents.
...
// Append-only audit log. Metadata only by default (textPreview can be set to 0
// to omit even the preview). A leaked audit log must not equal a leaked
// conversation.
...
// Keep this zero-dependency QR encoder in sync with scripts/simple-relay-server.mjs.
...
function decodePairComponent(value) {
  try {
...
<body>
  ...
  <script>
    ...
    function callbackDataToMessage(data) {
      ...
      const projects = data.match(/^legax:projects:([^:]+)(?::(\d+))?$/);
      if (projects) {
        const agentId = decodeURIComponent(projects[1]);
        selectAgent(agentId, true);
        return { targetAgentId: agentId, type: "control", action: "list_agent_projects", selectedAgentId: agentId, page: projects[2] ? Number(projects[2]) : undefined, text: "/projects " + agentId };
      }
