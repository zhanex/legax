...
let STORE_PATH = "";
let DESKTOP_SECRET = "";
let ALLOW_INSECURE_DEV = false;
let DEFAULT_SESSION = "default";
let MAX_REQUEST_BODY_BYTES = 1_048_576;
...
function standaloneConfigPath(args = [], env = process.env) {
  ...
  if (fs.existsSync(STANDALONE_CONFIG_PATH)) return STANDALONE_CONFIG_PATH;
  return requested ? path.resolve(requested) : STANDALONE_CONFIG_PATH;
}

function relayRuntimeOptions({
  args = [],
  env = process.env,
  mode = "managed"
} = {}) {
  const standalone = mode === "standalone";
  const configPath = standalone
    ? standaloneConfigPath(args, env)
    : resolveConfigPath(optionValue(args, "--config"), env);
  const configSourcePath = !standalone && !fs.existsSync(configPath)
    ? packageAssetPath("config.example.yaml")
    : configPath;
  const rawConfig = readYaml(configSourcePath) ?? {};
  const relayCfg = rawConfig.relay ?? {};
  const auditCfg = relayCfg.audit ?? {};
  const rawHost = String(relayCfg.host ?? "0.0.0.0");
  const desktopSecret = String(relayCfg.secret ?? "");
  const allowInsecureDev = relayCfg.allowInsecureDev === true;

  return {
    configPath,
    rawConfig,
    port: Number(relayCfg.port ?? 8787),
    host: (allowInsecureDev && !desktopSecret) ? "127.0.0.1" : rawHost,
    storePath: relayCfg.storePath
      ? (standalone ? path.resolve(relayCfg.storePath) : resolveRuntimeFile(relayCfg.storePath, configPath, "relay-store.json", env))
      : (standalone ? STANDALONE_STORE_PATH : resolveRuntimeFile("", configPath, "relay-store.json", env)),
    desktopSecret,
    allowInsecureDev,
    defaultSession: String(rawConfig.sessionId ?? "default"),
    maxRequestBodyBytes: Number(relayCfg.maxRequestBodyBytes ?? 1_048_576),
    auditDisabled: auditCfg.enabled === false,
    auditLogPath: auditCfg.path
...
function initializeRelayRuntime(options) {
  const runtime = relayRuntimeOptions(options);
  CONFIG_PATH = runtime.configPath;
  RAW_CONFIG = runtime.rawConfig;
  PORT = runtime.port;
  HOST = runtime.host;
  STORE_PATH = runtime.storePath;
  DESKTOP_SECRET = runtime.desktopSecret;
  ALLOW_INSECURE_DEV = runtime.allowInsecureDev;
  DEFAULT_SESSION = runtime.defaultSession;
  MAX_REQUEST_BODY_BYTES = runtime.maxRequestBodyBytes;
  AUDIT_DISABLED = runtime.auditDisabled;
  AUDIT_LOG_PATH = runtime.auditLogPath;
  AUDIT_MAX_TAIL = runtime.auditMaxTail;
  AUDIT_TEXT_PREVIEW = runtime.auditTextPreview;
